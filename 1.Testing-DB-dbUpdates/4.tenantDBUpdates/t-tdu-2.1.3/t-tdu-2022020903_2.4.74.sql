...
UPDATE vp_bill bill SET bill.ocr_running_status='N'
WHERE bill.bill_type='O' AND bill.ocr_running_status IS NULL;

ALTER TABLE `hst_vp_bill`
	ADD COLUMN `ocr_running_status` CHAR(1) NULL DEFAULT NULL COLLATE 'latin1_swedish_ci' AFTER `inbox_attachment_id`,
	ADD COLUMN `ocr_running_date` DATE NULL DEFAULT NULL AFTER `ocr_running_status`;
	
UPDATE hst_vp_bill hst SET hst.ocr_running_status='N'
WHERE hst.bill_type='O' AND hst.ocr_running_status IS NULL;
