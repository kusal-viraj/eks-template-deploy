...
UPDATE `common_grid_column_user_wice` SET `drop_down_url_id` = 37 WHERE `grid_id` = 59 AND `field` = 'incompleteDetail.tpCompanyId';
UPDATE `common_grid_column_user_wice` SET `drop_down_url_id` = 37 WHERE `grid_id` = 21 AND `field` = 'syncDetail.tpCompanyId';